...
//     // Fetch card details using CustomerID
//     const [cardDetails] = await db.query(
//       `
//       SELECT ac.AccountID, ac.CardType, ac.PIN, a.Balance 
//       FROM Customers c
//       LEFT JOIN Accounts a ON c.CustomerID = a.CustomerID
//       LEFT JOIN Account_Cards ac ON a.AccountID = ac.AccountID
//       WHERE c.CustomerID = ?
//       `,
//       [CustomerID]
//     );

//     // Validate if card details were found
//     if (!cardDetails || cardDetails.length === 0) {
//       return res.status(404).json({
//         success: false,
//         message: `Invalid Customer ID: ${CustomerID}.`,
...
//       return res.status(404).json({
//         success: false,
//         message: `Customer with ID ${CustomerID} does not have an account.`,
//       });
//     }
...
//     const [updatedAccount] = await db.query(
//       `
//       SELECT ac.AccountID, ac.CardType, a.Balance, c.CustomerName
//       FROM Account_Cards ac
//       JOIN Accounts a ON ac.AccountID = a.AccountID
//       JOIN Customers c ON a.CustomerID = c.CustomerID
//       WHERE ac.AccountID = ?
...
//       return res.status(500).json({
//         success: false,
//         message: "Failed to retrieve updated account details.",
//       });
//     }
...
//     return res.status(200).json({
//       success: true,
//       message: `Successfully deposited ₹${Amount} to Account ID: ${card.AccountID}.`,
//       updatedDetails: updatedAccount[0],
//     });
...
const withdrawMoney = async (req, res) => {
  try {
    ...
    if (!cardDetails || cardDetails.length === 0) {
      return res.status(404).json({
        success: false,
        message: `Invalid Customer ID ${CustomerID} or no associated account/card found.`,
      });
    }
